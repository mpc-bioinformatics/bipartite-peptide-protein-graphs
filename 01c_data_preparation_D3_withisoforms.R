...
DATA <- read.table("data/D3/D3_quant/peptides.txt",
                   sep = "\t",
...
fasta <- read.fasta(file = "data/D3/D3_fasta/20210413_uniprot-proteome UP000005640_HomoSapiens_withIsoforms_v2021_02.fasta",
                    seqtype = "AA", as.string = TRUE)
fasta_vec <- unlist(fasta)
protein_accessions <- strsplit2(attr(fasta, "name"), "\\|")[,2]

fasta_EColi <- read.fasta(file = "data/D3/D3_fasta/20210413_uniprot-proteome UP000000625_EColi_withIsoforms_v2021_02.fasta",
                         seqtype = "AA", as.string = TRUE)
fasta_vec <- c(fasta_vec, unlist(fasta_EColi))
protein_accessions <- c(protein_accessions, strsplit2(attr(fasta_EColi, "name"), "\\|")[,2])
fasta_contaminant <- read.fasta(file = "data/D3/D3_fasta/MaxQuant_contaminants_downloaded_20200527.fasta",
                                seqtype = "AA", as.string = TRUE)
...
write.table(D, "data/D3/D3_quant/preprocessed_peptide_data_D3.txt",
            sep = "\t", row.names = FALSE)
...
DATA <- read.table(file = "data/D3/D3_quant/preprocessed_peptide_data_D3.txt", sep = "\t", header = TRUE)
matrix_01 <- generate_01_matrix(DATA$peptide, DATA$protein)
saveRDS(matrix_01, "data/D3/D3_quant/preprocessed/01Matrix.rds")


################################################################################
#### Step 5) find sub matrices / connected components ####

matrix_01 <- readRDS("data/D3/D3_quant/preprocessed/01Matrix.rds")
RES <- generate_submatrices(matrix_01)
saveRDS(RES$Submatrix, file = "data/D3/D3_quant/preprocessed/Submatrix.rds")
saveRDS(RES$Subgraphs, file = "data/D3/D3_quant/preprocessed/Subgraphs.rds")


################################################################################
#### Step 6) collapse protein nodes ####

S <- readRDS("data/D3/D3_quant/preprocessed/Submatrix.rds")
S_proteingroups <- form_proteingroups(S$Submatrix)
saveRDS(S_proteingroups, file = "data/D3/D3_quant/preprocessed/Submatrix_proteinGroups.rds")


################################################################################
#### Step 7) calculate peptide ratios / fold changes ####

D <- read.table(file = "data/D3/D3_quant/preprocessed_peptide_data_D3.txt", header = TRUE)
...
write.table(D_mean_NA_mind2, file = "data/D3/D3_quant/preprocessed/D_mean_NA_mind2.txt",
            row.names = FALSE, sep = "\t")

### one missing value allowed:
D_aggr <- read.table("data/D3/D3_quant/preprocessed/D_mean_NA_mind2.txt", sep = "\t", header = TRUE)
FC_1_2 <- foldChange(D_aggr, "state1", "state2")

FC <- FC_1_2
FC <- as.data.frame(FC)
FC <- cbind(D_aggr[, 1:2], FC)
write.table(FC, file = "data/D3/D3_quant/preprocessed/FC_NA_mind2.txt", row.names = FALSE, sep = "\t")


################################################################################
#### Step 8) re-calculate sub matrices but only with peptides with valid ratios per pairwise comparison ####

FC.table <- read.table(file = "data/D3/D3_quant/preprocessed/FC_NA_mind2.txt", header = TRUE, sep = "\t")
...
save(list =  ls(pattern = "Submatrix"),
     file = "data/D3/D3_quant/preprocessed/Submatrices.RData")
...
save(list =  ls(pattern = "Submatrix_merged"),
     file = "data/D3/D3_quant/preprocessed/Submatrices_merged_Peptides.RData")
