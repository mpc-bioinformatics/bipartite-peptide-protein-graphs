...
DATA <- read.table("data/D2_without_isoforms/D2_quant/peptides.txt",
                    sep = "\t",
...
fasta <- read.fasta(file = "data/D2_without_isoforms/D2_fasta/2020_01_31_proteome_S_cerevisae.fasta",
                    seqtype = "AA", as.string = TRUE)
fasta_vec <- unlist(fasta)
protein_accessions <- strsplit2(attr(fasta, "name"), "\\|")[,2]
fasta_UPS1 <- read.fasta(file = "data/D2_without_isoforms/D2_fasta/ups1-ups2-sequences.fasta",
                         seqtype = "AA", as.string = TRUE)
fasta_vec <- c(fasta_vec, unlist(fasta_UPS1))
protein_accessions <- c(protein_accessions, strsplit2(attr(fasta_UPS1, "name"), "\\|")[,1])
fasta_contaminant <- read.fasta(file = "data/D2_without_isoforms/D2_fasta/MaxQuant_contaminants_downloaded_20200527.fasta",
                                seqtype = "AA", as.string = TRUE)
...
write.table(D, "data/D2_without_isoforms/D2_quant/preprocessed_peptide_data_D2.txt",
            sep = "\t", row.names = FALSE)

################################################################################
#### Step 4) calculate 0/1 biadjacency matrix matrix ####

DATA <- read.table(file = "data/D2_without_isoforms/D2_quant/preprocessed_peptide_data_D2.txt", sep = "\t", header = TRUE)
matrix_01 <- generate_01_matrix(DATA$peptide, DATA$protein)
saveRDS(matrix_01, "data/D2_without_isoforms/D2_quant/preprocessed/01Matrix.rds")


################################################################################
#### Step 5) find sub matrices / connected components ####

matrix_01 <- readRDS("data/D2_without_isoforms/D2_quant/preprocessed/01Matrix.rds")
RES <- generate_submatrices(matrix_01)
saveRDS(RES$Submatrix, file = "data/D2_without_isoforms/D2_quant/preprocessed/Submatrix.rds")
saveRDS(RES$Subgraphs, file = "data/D2_without_isoforms/D2_quant/preprocessed/Subgraphs.rds")


################################################################################
#### Step 6) collapse protein nodes ####

S <- readRDS("data/D2_without_isoforms/D2_quant/preprocessed/Submatrix.rds")
S_proteingroups <- form_proteingroups(S$Submatrix)
saveRDS(S_proteingroups, file = "data/D2_without_isoforms/D2_quant/preprocessed/Submatrix_proteinGroups.rds")


################################################################################
#### Step 7) calculate peptide ratios / fold changes ####

D <- read.table(file = "data/D2_without_isoforms/D2_quant/preprocessed_peptide_data_D2.txt", header = TRUE)
...
write.table(D_mean_NA_mind2, file = "data/D2_without_isoforms/D2_quant/preprocessed/D_mean_NA_mind2.txt",
            row.names = FALSE, sep = "\t")

### one missing value allowed:
D_aggr <- read.table("data/D2_without_isoforms/D2_quant/preprocessed/D_mean_NA_mind2.txt", sep = "\t", header = TRUE)
...
write.table(FC, file = "data/D2_without_isoforms/D2_quant/preprocessed/FC_NA_mind2.txt", row.names = FALSE, sep = "\t")


################################################################################
#### Step 8) re-calculate sub matrices but only with peptides with valid ratios per pairwise comparison ####

FC.table <- read.table(file = "data/D2_without_isoforms/D2_quant/preprocessed/FC_NA_mind2.txt", header = TRUE, sep = "\t")
...
save(list =  ls(pattern = "Submatrix"),
     file = "data/D2_without_isoforms/D2_quant/preprocessed/Submatrices.RData")
...
save(list =  ls(pattern = "Submatrix_merged"),
     file = "data/D2_without_isoforms/D2_quant/preprocessed/Submatrices_merged_Peptides.RData")
